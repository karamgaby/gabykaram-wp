		...
		public function plugin_api( $start_instance = false ) {
			...
			if ( file_exists( $file ) ) {
				...
				if ( class_exists( 'vxcf_plugin_api' ) ) {
					...
					self::$plugin   = new vxcf_plugin_api( self::$id,
						self::$version,
						self::$type,
						$this->domain,
						$update_id,
			...
			if ( is_array( $lead ) && count( $lead ) > 0 ) {
				...
				if ( ! empty( $fields ) ) {
					foreach ( $lead as $k => $v ) {
						...
						} elseif ( ! in_array( $type, array( 'file', 'mfile' ) ) ) {
...
//var_dump($post_data); die();
			...
			if ( is_array( $post_data ) ) {
				foreach ( $post_data as $k => $val ) {
					if ( in_array( $k, array( 'vx_width', 'vx_height', 'vx_url', 'g-recaptcha-response' ) ) ) {
						continue;
					}
					if ( isset( $tags[ $k ] ) ) {
				...
				foreach ( $fields as $field ) {
					...
					if ( isset( $entry[ $id ] ) ) {
						...
						if ( isset( $field['type'] ) ) {
							...
							} elseif ( in_array( $field['type'], array( 'radio', 'multiselect' ) ) ) {
								$val = explode( ',', $val );
							}
						}
						if ( ! empty( $val ) ) {
...
//error_reporting(E_ALL);
		...
		public function get_all_fields( $form_id ) {
			...
			$this->fields = $fields = apply_filters( 'vx_mapping_standard_fields',
				array( 'gf' => array( "title" => __( 'Contact Form Fields', 'contact-form-entries' ), "fields" => $fields ) ) );
...
//var_dump($leads,$fields);die();
		...
		public static function display_msg( $type, $message, $id = "" ) {
			...
			if ( $type == "admin" ) {
				?>
              <div class="error vx_notice below-h2 notice is-dismissible" data-id="<?php
			  echo esc_attr( $id ) ?>"><p><span class="dashicons dashicons-megaphone"></span> <b><?php
						  _e( 'Contact Form Entries Plugin', 'contact-form-entries' ) ?>. </b> <?php
  ...
  <div style="background-color: rgba(224, 224, 224, 0.5);  padding: 9px; margin: 0px 10px 10px 28px "><div style="background-color: #d54d21; padding: 5px 10px; color: #fff"
		...
		public static function get_forms() {
			...
			if ( defined( 'ELEMENTOR_PRO_VERSION' ) ) {  //&& class_exists('ElementorPro\\Plugin')
				global $wpdb;
				$data      = $wpdb->get_results( "SELECT m.post_id, m.meta_value,p.post_title FROM $wpdb->postmeta m inner join $wpdb->posts p on(m.post_id=p.ID) WHERE p.post_status='publish' and m.meta_key = '_elementor_data' limit 70",
					ARRAY_A ); //__elementor_forms_snapshot
				$forms_arr = array();
				...
				case'na':
					if ( class_exists( 'Ninja_Forms' ) ) {
						...
						foreach ( $form_fields as $obj ) {
							...
							if ( in_array( $type, array( 'spam', 'confirm', 'submit', 'repeater', 'save', 'html', 'hr' ) ) ) {
								continue;
							} //|| !isset($field['required'])  // it is not set for hidden fields that is why removed it
							if ( $type == 'checkbox' ) {
					...
					for ( $i = 1; $i < 500; $i ++ ) {
						if ( isset( $settings[ 'form' . $id ][ 'cforms' . $id . '_count_field_' . $i ] ) ) {
							...
							if ( in_array( $field_type, array( 'checkbox', 'checkboxgroup' ) ) ) {
								$field_type = 'checkbox';
							} elseif ( in_array( $field_type, array( 'selectbox', 'ccbox', 'emailtobox' ) ) ) {
								$field_type = 'select';
							} elseif ( in_array( $field_type, array( 'multiselectbox' ) ) ) {
								$field_type = 'multiselect';
							} elseif ( in_array( $field_type, array( 'radiobuttons' ) ) ) {
								$field_type = 'radio';
							} elseif ( in_array( $field_type, array( 'upload' ) ) ) {
								$field_type = 'file';
							}
							if ( ! empty( $field_name ) ) {
							...
							if ( ! empty( $form['form_fields'] ) ) {
								foreach ( $form['form_fields'] as $tag ) {
									if ( ! empty( $tag['custom_id'] ) ) {
										if ( empty( $tag['field_type'] ) ) {
											$tag['field_type'] = $tag['custom_id'];
										}
				...
				case'wc':
					...
					foreach ( $arr as $k => $v ) {
						...
						if ( $k == 'billing_email' ) {
							...
							$field['type'] = 'tel';
						} elseif ( in_array( $k, array( 'billing_address_1', 'billing_address_2', 'shipping_address_1', 'shipping_address_2', 'order_note' ) ) ) {
							$field['type'] = 'textarea';
						} elseif ( in_array( $k, array( 'billing_state', 'shipping_state' ) ) ) {
							$field['type'] = 'state';
						} elseif ( in_array( $k, array( 'billing_country', 'shipping_country' ) ) ) {
							$field['type'] = 'country';
						}
						$fields[] = $field;
					}
						...
						if ( ! empty( $forms_arr ) ) {
							...
							foreach ( $form['fields'] as $v ) {
								...
								if ( in_array( $type, array( 'radio', 'checkbox', 'select' ) ) ) {
									...
									if ( in_array( $v['type'], array( 'payment-select', 'payment-multiple' ) ) ) {
										$is_val = true;
									} //|| (isset($v['show_values'])&& $v['show_values'] ='1' ) front form submission always sends label , not value , we will have to find value from form fields and store real value
									$choices = array();
									if ( ! empty( $v['choices'] ) ) {
