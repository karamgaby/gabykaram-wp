...
class Axio_Core_Dashboard_Recent_Widget extends Axio_Core_Sub_Feature {
  ...
  public function setup() {
    ...
    // var: key
    $this->set('key', 'x_core_dashboard_recent_widget');
  ...
  public function run() {
    ...
    add_action('wp_dashboard_setup', array($this, 'register_x_recent_dashboard_widget'));
    add_action('admin_enqueue_scripts', array($this, 'x_recent_dashboard_widget_styles'));
   ...
   */
  public static function x_recent_dashboard_widget_display() {
    ...
    if (!empty($user_posts)) :
      ?>
      <div class="x-recent-section">
        <h3><?php echo esc_attr__('My content and changes', 'x-core'); ?></h3>
        <ul>
      <?php
      ...
      ?>
        <li><span class="x-recent-time"><?php echo esc_html(date_format($modified_time, "$date_format $time_format")); ?></span><span class="x-recent-link"><?php edit_post_link($title, '', '', $user_posts[$i]->ID); ?></span></li>
        <?php
      }
        ?>
        </ul>
    ...
    <div class="x-recent-section">
      <h3><?php echo esc_attr__('Recent edits', 'x-core'); ?></h3>
      <ul>
      <?php
      ...
      ?>
        <li><span class="x-recent-time"><?php echo esc_html(get_the_modified_date("$date_format $time_format")); ?></span><span class="x-recent-link"><?php edit_post_link($title, '', '', $query->post->ID); ?></span></li>
        <?php
      endwhile;
      ?>
      </ul>
   ...
   */
  public static function x_core_order_posts_array_by_modified_date($a, $b) {
