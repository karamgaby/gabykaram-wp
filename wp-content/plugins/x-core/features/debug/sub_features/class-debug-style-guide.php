 ...
 */
class X_Core_Debug_Style_Guide extends X_Core_Sub_Feature {

  public function setup() {

    // var: key
    $this->set('key', 'x_core_debug_style_guide');
   ...
   */
  public function run() {
    add_filter('the_content', array($this, 'x_core_style_guide_markup'));
  }

  /**
   * Add style guide test markup if the GET parameter "?ac-debug=styleguide" is present in the url.
   * The x_core_custom_markup filter makes it possible to replace the default with custom markup
   *
   * @param string content from the_content()
   *
   * @return string style guide or custom markup
   */
  public static function x_core_style_guide_markup($content) {
