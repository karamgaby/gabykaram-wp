...
class X_Core_Plugins_Acf extends X_Core_Sub_Feature {
  ...
  public function setup() {
    ...
    // var: key
    $this->set('key', 'x_core_plugins_acf');
  ...
  public function run() {
    ...
    add_filter('acf/settings/show_admin', array($this, 'x_core_hide_acf_from_nonadmins'));
    add_filter('render_block_data', array($this, 'x_core_acf_render_block_data'), 1, 2);
   ...
   */
  public static function x_core_hide_acf_from_nonadmins($show) {
   ...
   */
  public static function x_core_recursive_block_mode_changer(&$parsed_block, $forced_block_types) {
    ...
    if (isset($parsed_block['innerBlocks']) && !empty($parsed_block['innerBlocks'])) {
      foreach ($parsed_block['innerBlocks'] as &$inner_block) {
        self::x_core_recursive_block_mode_changer($inner_block, $forced_block_types);
      }
    }
   ...
   * @return array parsed block data
   */
  public static function x_core_acf_render_block_data($parsed_block, $source_block) {

    $forced_block_types = apply_filters('x_core_force_block_type_mode', array());

    if (is_array($forced_block_types) && !empty($forced_block_types)) {
      self::x_core_recursive_block_mode_changer($parsed_block, $forced_block_types);
    }
