...
class DashboardRecentWidgetTest extends WP_UnitTestCase {
  ...
  public function test_dashboard_recent_widget() {
    $class = $this->dash->get_sub_features()['x_core_dashboard_recent_widget'];
    // key
    $this->assertNotEmpty(
       $class->get_key()
    );
    ...
    // check action hooks
    $this->assertSame(
      10, has_action('wp_dashboard_setup', array($class, 'register_x_recent_dashboard_widget'))
    );
    $this->assertSame(
      10, has_action('admin_enqueue_scripts', array($class, 'x_recent_dashboard_widget_styles'))
    );
    ...
    $class->register_x_recent_dashboard_widget();

    // check that the meta box is present
    $this->assertArrayHasKey(
      'x_recent_dashboard_widget', $wp_meta_boxes['dashboard']['side']['high']
    );
    ...
    // run callback function
    $class->x_recent_dashboard_widget_display();
    ...
    add_filter('x_core_recent_widget_user_blacklist', function ($array) {
    ...
    $this->assertGreaterThanOrEqual(
      0, $class->x_core_order_posts_array_by_modified_date(get_post($post1), get_post($post3))
    );
    ...
    // run callback function
    $class->x_recent_dashboard_widget_styles('test.php');
    ...
    // run callback function
    $class->x_recent_dashboard_widget_styles('index.php');
    // check that the styles are included (queued)
    $this->assertSame(
      'x_core-dashboard-widget-styles', $wp_styles->queue[0]
    );
  }
