...
class SecurityRemoveCommentModerationTest extends WP_UnitTestCase {
  ...
  public function test_security_remove_comment_moderation() {
    $class = $this->security->get_sub_features()['x_core_security_remove_comment_moderation'];
    // key
    $this->assertNotEmpty(
       $class->get_key()
    );
    ...
    $this->assertSame(
      array('admin@admin.admin'), $class->x_core_comment_moderation_post_author_only($emails, $comment)
    );
    ...
    // check that the callback function returns correct value
    $this->assertSame(
      array('user@user.user'), $class->x_core_comment_moderation_post_author_only($emails, $comment)
    );
  }
}
