...
class SecurityHeadCleanupTest extends WP_UnitTestCase {
  ...
  public function test_security_head_cleanup() {
    $class = $this->security->get_sub_features()['x_core_security_head_cleanup'];
    // key
    $this->assertNotEmpty(
       $class->get_key()
    );
    ...
    $this->assertSame(
      10, has_filter('wp_headers', array($class, 'x_core_remove_pingback_header'))
    );
    $this->assertSame(
    ...
    // check that the callback function has removed the correct value
    $this->assertArrayNotHasKey(
      'pingback.ping', $class->x_core_remove_pingback_functionality($methods)
    );
