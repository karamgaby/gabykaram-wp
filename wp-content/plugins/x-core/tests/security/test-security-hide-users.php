...
class SecurityHideUsersTest extends WP_UnitTestCase {
  ...
  public function test_security_hide_users() {
    $class = $this->security->get_sub_features()['x_core_security_hide_users'];
    // key
    $this->assertNotEmpty(
       $class->get_key()
    );
    ...
    // check filter hooks
    $this->assertSame(
      100, has_filter('the_author', array($class, 'x_core_rename_authors'))
    );
    $this->assertSame(
      100, has_filter('the_modified_author', array($class, 'x_core_rename_authors'))
    );
    $this->assertSame(
      100, has_filter('get_the_author_link', array($class, 'x_core_author_link_to_front_page'))
    );
    $this->assertSame(
    ...
    $this->assertSame(
      get_bloginfo('name'), $class->x_core_rename_authors($name)
    );
    ...
    $this->assertArrayHasKey(
      '/wp/v2/users', $class->x_core_disable_user_endpoints($endpoints)
    );
