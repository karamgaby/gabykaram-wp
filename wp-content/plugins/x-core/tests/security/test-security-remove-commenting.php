...
class SecurityRemoveCommentingTest extends WP_UnitTestCase {
  ...
  public function test_security_remove_commenting() {
    $class = $this->security->get_sub_features()['x_core_security_remove_commenting'];
    // key
    $this->assertNotEmpty(
       $class->get_key()
    );
    ...
    $this->assertSame(
      10, has_action('admin_init', array($class, 'x_core_disable_comments_post_types_support'))
    );
    $this->assertSame(
      10, has_action('admin_menu', array($class, 'x_core_disable_comments_admin_menu'))
    );
    $this->assertSame(
      10, has_action('admin_init', array($class, 'x_core_disable_comments_admin_menu_redirect'))
    );
    $this->assertSame(
      10, has_action('admin_init', array($class, 'x_core_disable_comments_dashboard'))
    );
    $this->assertSame(
      10, has_action('wp_before_admin_bar_render', array($class, 'x_core_admin_bar_render'))
    );
    $this->assertSame(
      10, has_filter('comments_array', array($class, 'x_core_disable_comments_hide_existing_comments'))
    );
    $this->assertSame(
    ...
    // run callback function
    $class->x_core_disable_comments_admin_menu();
    ...
    // run callback function
    $class->x_core_disable_comments_dashboard();
    ...
    $this->assertSame(
      array(), $class->x_core_disable_comments_hide_existing_comments($comments)
    );
  }
