...
namespace core\TGMPA-php\core\TGMPA;

use _test\core\stdClass;
use _test\core\WP_Error;
use _test\coreuse;

_test\coreuse _test\coreuse _test\core/**
...
//          die(var_dump( __NAMESPACE__ . '\TGMPA_Utils'));
        ...
        unset( $type, $linked_plugins, $count, $last_plugin, $imploded );
    ...
    foreach ( $keys as $key ) {
      if ( isset( $config[ $key ] ) ) {
        if ( is_array( $config[ $key ] ) ) {
          foreach ($config[$key] as $subkey => $subvalue) {
            if (is_string($subkey)) {
 ...
 *
  ...
  public function update_dismiss() {
    ...
    foreach ( $all_users as $user ) {
      $user_id = $user->ID;
      delete_metadata( 'user', $user_id, 'tgmpa_dismissed_notice_' . $this->id, '', true );
    }
  }
  ...
  public static function get_instance() {
	...
	public static function getInstance(): TGM_Plugin_Activation {
