...
function x_wpcf7_text_form_tag_handler( $tag ) {
  if ( empty( $tag->name ) ) {
    return '';
  }
...
add_action(
  ...
  'x_wpcf7_swv_add_text_rules',
  10, 2
);

function x_wpcf7_swv_add_text_rules( $schema, $contact_form ) {
  $tags = $contact_form->scan_form_tags( array(
    'basetype' => array( 'x_text', 'x_email', 'x_url', 'x_tel' ),
  ) );
...
add_filter( 'wpcf7_messages', 'x_wpcf7_text_messages', 10, 1 );

function x_wpcf7_text_messages( $messages ) {
...
add_action( 'wpcf7_admin_init', 'x_wpcf7_add_tag_generator_text', 15, 0 );

function x_wpcf7_add_tag_generator_text() {
  $tag_generator = WPCF7_TagGenerator::get_instance();
  $tag_generator->add( 'x_text', __( 'X text', 'contact-form-7' ),
    'x_wpcf7_tag_generator_text' );
  $tag_generator->add( 'x_email', __( 'X email', 'contact-form-7' ),
    'x_wpcf7_tag_generator_text' );
  $tag_generator->add( 'x_url', __( 'X URL', 'contact-form-7' ),
    'x_wpcf7_tag_generator_text' );
  $tag_generator->add( 'x_tel', __( 'X tel', 'contact-form-7' ),
    'x_wpcf7_tag_generator_text' );
}

function x_wpcf7_tag_generator_text( $contact_form, $args = '' ) {
  $args = wp_parse_args( $args, array() );
  $type = $args['id'];
  if ( ! in_array( $type, array( 'x_email', 'x_url', 'x_tel' ) ) ) {
    $type = 'x_text';
  }
